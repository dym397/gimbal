...
## 2026-07-18 云台0.2度到位阈值回归

- [x] 将 `GIMBAL_SETTLE_THRESHOLD` 从 `0.3°` 改为 `0.2°`。
- [x] 对入队目标、控制线程目标、反馈角、抢占差值和到位误差执行一位小数量化。
- [x] 控制命令/目标/反馈/误差日志改为一位小数显示。
- [ ] 实机统计 `GIMBAL_SETTLED`、分轴 `GIMBAL_CMD_RETRY` 和 `GIMBAL_TIMEOUT`，确认机械回差下两轴均可稳定进入 `<0.2°`。

## 2026-07-18 SFL0603现场验证待办

### 已完成
- [x] 将云台画面对准点统一改为激光光斑标定位置 `(1290,724)`，并在实时预览绘制准星。
- [x] 接入 `SFL0603` 协议驱动，启动时确认待机且默认禁止出光。
- [x] 实现对准后10Hz按需测距、首个有效结果后停止、运动/失稳撤销和1.5秒硬超时。
- [x] 将SFL协议、产品说明和驱动说明复制到板端 `docs/sfl0603/` 并校验SHA-256。
- [x] 完成19项无硬件测试、协议示例帧校验、静态检查和准星渲染检查。

### 待现场完成
- [ ] 在确认目标距离大于15米且不是玻璃、白墙等强反射材质后，进行首次真实出光测试。
- [ ] 抓取并核对 `LASER_SESSION_START`、`LASER_FIRST_VALID_RESULT`、`LASER_SESSION_STOP` 的时间顺序，确认每次只启动一次且STOP有应答。
- [ ] 验证实际RS422设备路径、115200/8N1、100ms周期和距离单位0.1m。
- [ ] 故障注入无回波、APD异常、云台中途运动和STOP应答丢失，确认均能安全停止且不会自动重启。
