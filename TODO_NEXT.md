...
### 已完成
- [x] `logic_id=12` 的SORT投影Y补偿设为 `+46.5px`。
- [x] `logic_id=13` 的SORT投影Y补偿设为 `+306.5px`。
- [x] 其他摄像头默认补偿 `0px`。
- [x] 视觉关联改为补偿后Y轴绝对残差的匈牙利一对一分配，不设置硬门限；X轴只记录、不参与匹配。
- [x] 关联日志增加来源摄像头、补偿前后Y、原始X/Y偏差和实际Y轴代价。

### 下一步现场验证
1. 让目标依次在每个固定摄像头画面内稳定停留，按 `sort_source_logic_id` 分段统计 `detection_center_y - sort_projected_y`。
2. 对 `logic_id=12/13` 检查 `association_cost_y_px` 是否较补偿前明显收敛，并确认补偿方向没有反向。
3. 对其他摄像头从稳定单目标片段估计中位Y偏差，再决定是否加入新的补偿量。
