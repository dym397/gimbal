...
## 2026-07-31 按摄像头补偿Y轴的视觉关联
- 视觉测距结果与SORT投影的最终回填改为“按来源摄像头补偿SORT投影Y坐标，再以Y轴绝对残差构造匈牙利代价”；X轴不参与匹配，仅保留在日志中用于诊断。
- 当前已知补偿量：`logic_id=12` 为 `+46.5px`，`logic_id=13` 为 `+306.5px`；正号表示将SORT投影Y向画面下方修正。其余 `logic_id` 默认补偿 `0px`。
- 当前仍执行无硬门限的一对一匈牙利分配：单SORT/单检测直接绑定，多目标按补偿后的Y残差取得全局最小组合；Y位置接近时仍可能错绑，本轮不增加歧义拒绝或回退状态机。
