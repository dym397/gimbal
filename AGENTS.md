...
## 2026-07-18 云台0.2度到位阈值与一位小数量化
- `GIMBAL_SETTLE_THRESHOLD` 已从 `0.3°` 收紧为 `0.2°`；方位和俯仰误差必须同时严格小于 `0.2°` 并持续 `0.20s` 才置为 `is_settled=True`，恰好 `0.2°` 仍视为未到位并可触发按轴重发。
- 所有云台目标在入队时量化为 `0.1°`，控制线程取出初始/抢占目标时再次量化，GT06Z驱动编码前第三次量化；目标变量不会保留协议无法发送的多位小数。
- 编码器反馈、抢占角差和到位误差也在比较前统一量化为一位小数，避免二进制浮点在 `0.2°` 边界产生误判。
- 云台命令、目标、反馈和误差的控制日志统一显示一位小数；跟踪器内部预测精度不因本次修改而降低。
- 风险：阈值收紧后更容易暴露机械回差或增加 `GIMBAL_TIMEOUT`；实机回归需统计到位、重发和超时比例，并确认两轴均能稳定进入 `0.1°` 误差。

## 2026-07-18 主业务移除IMU
- `core/main_tracking_v9.py` 已删除IMU串口默认值、环境配置、端口校验、HWT905初始化、主循环读取打印和退出关闭逻辑；主业务不再导入或访问IMU。
- `core/hwt905_driver.py` 作为独立底层驱动完整保留且未修改，后续如需单独联调或重新接入仍可使用。
- 本次不改变云台、激光、GPS、摄像头、UDP/UI协议、跟踪或打击逻辑。

## 2026-07-18 板端设备物理端口固化
- Linux板端的云台、SFL0603激光、GPS和云台摄像头统一使用 `/dev/serial/by-path/` 或 `/dev/v4l/by-path/`，禁止回退到会随枚举顺序变化的 `/dev/ttyUSB*`、`/dev/video*`。
- 摄像头直连板端USB端口：`/dev/v4l/by-path/platform-xhci-hcd.4.auto-usb-0:1.1:1.0-video-index0`。
- SFL0603激光直连板端USB端口：`/dev/serial/by-path/platform-xhci-hcd.4.auto-usb-0:1.4:1.0-port0`。
- GPS连接USB HUB下游1号端口：`/dev/serial/by-path/platform-xhci-hcd.4.auto-usb-0:1.2.1:1.0-port0`。
- 云台连接同一USB HUB下游2号端口：`/dev/serial/by-path/platform-xhci-hcd.4.auto-usb-0:1.2.2:1.0-port0`。
- 三个串口转换器均为没有唯一序列号的CH341，`/dev/serial/by-id/usb-1a86_USB_Serial-if00-port0` 无法区分角色；固化依赖物理插口，禁止交换GPS、云台、激光的USB插口。
- 代码默认值与 `/etc/default/gimbal-tracking` 已同步。验证过程只做udev枚举，服务保持inactive，未打开激光/云台串口。

## 2026-07-18 SFL0603按需测距与激光光斑准星
- `single-laser` 分支的云台相机瞄准基准已从几何中心 `(1280,720)` 改为标定后的激光光斑位置 `(1290,724)`，即向右 `10px`、向下 `4px`；稳定判断、30/50px死区/触发门限、像素角度换算、诊断日志和实时预览准星统一使用该位置。
- 旧 `SDDMLaser` 已由 `core/sfl0603_driver.py` 的 `SFL0603` 替换。协议和安全资料保存在 `docs/sfl0603/`，协议实现以 `SFL系列铒玻璃通讯协议V0.005.pdf` 为准。
- SFL0603不再常开：初始化只打开串口、发送 `0x00` 并确认待机；只有唯一YOLO目标连续6帧稳定且进入标定准星 `±30px` 后，才临时解锁并发送 `0x02`、周期 `100ms` 的10Hz连续测距命令。
- 收到第一个有效距离帧后，必须先确认 `0x00` 停止成功，再把距离以 `sfl0603_laser` 来源写入当前master；目标失稳、云台运动、目标切换、读取异常或1.5秒无有效结果均会停止并锁回待机。同一对准事件完成或超时后不会自动重启。
- 停止命令最多重试3次；仍无法确认停止时关闭驱动并结束读取线程，不允许继续发起新的出光会话。
- 产品说明明确禁止对15米以内的玻璃、白墙等强反射目标发射激光，以防回波过强永久损坏APD。实机验证前必须人工确认目标距离、材质、接收镜头和现场安全条件。
- 软件验证：核心文件 `py_compile`、`git diff --check`、19项无硬件断言和协议示例帧检查均通过；实时准星渲染已确认位于 `(1290,724)`。本轮未启动systemd服务，未打开真实激光或进行出光测试。
