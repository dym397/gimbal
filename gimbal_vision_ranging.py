#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CPU-only gimbal-camera YOLO ranging service.
...
- YOLO receives original-resolution 640x640 dynamic crops, never a resized full frame;
...
- bbox coordinates are restored to the 2560x1440 camera coordinate system before
  entering the distance model.
"""
...
def _parse_camera_source(value: str):
    ...
    if text.lstrip("+-").isdigit():
        return int(text), cv2.CAP_ANY, f"index:{int(text)}"
    return text, cv2.CAP_ANY, text
...
class _CpuYoloDetector:
    """YOLOv5 detector fixed to torch CPU."""
